...
use crate::values::{
    ...
    ScalableVectorValue, StructValue, TokenValue, VectorValue,
};
...
enum_value_set! {AnyValueEnum: ArrayValue, IntValue, FloatValue, PhiValue, FunctionValue, PointerValue, StructValue, VectorValue, ScalableVectorValue, TokenValue, InstructionValue, MetadataValue}
enum_value_set! {BasicValueEnum: ArrayValue, IntValue, FloatValue, PointerValue, StructValue, VectorValue, ScalableVectorValue, TokenValue}
enum_value_set! {BasicMetadataValueEnum: ArrayValue, IntValue, FloatValue, PointerValue, StructValue, VectorValue, ScalableVectorValue, TokenValue, MetadataValue}
...
impl<'ctx> AnyValueEnum<'ctx> {
    ...
    pub fn is_token_value(self) -> bool {
        matches!(self, AnyValueEnum::TokenValue(_))
    }
    ...
    pub fn into_token_value(self) -> TokenValue<'ctx> {
        if let AnyValueEnum::TokenValue(v) = self {
            ...
            panic!("Found {:?} but expected the TokenValue variant", self)
        }
    }
}
...
impl<'ctx> BasicValueEnum<'ctx> {
    ...
    pub fn get_name(&self) -> &CStr {
        match self {
            ...
            BasicValueEnum::TokenValue(v) => v.get_name(),
        }
    }
    ...
    pub fn set_name(&self, name: &str) {
        match self {
            ...
            BasicValueEnum::TokenValue(v) => v.set_name(name),
        }
    }
    ...
    pub fn is_token_value(self) -> bool {
        matches!(self, BasicValueEnum::TokenValue(_))
    }
...
impl<'ctx> BasicMetadataValueEnum<'ctx> {
    pub(crate) unsafe fn new(value: LLVMValueRef) -> Self {
        match LLVMGetTypeKind(LLVMTypeOf(value)) {
            ...
            #[cfg(not(any(feature = "llvm3-6", feature = "llvm3-7")))]
            LLVMTypeKind::LLVMTokenTypeKind => BasicMetadataValueEnum::TokenValue(TokenValue::new(value)),
    ...
    pub fn is_token_value(self) -> bool {
        matches!(self, BasicMetadataValueEnum::TokenValue(_))
    }
